...
# def get_scope_identity():
#     try:
#         with engine.connect() as conn:
#             result = conn.execute(text("SELECT SCOPE_IDENTITY()"))
#             return result.scalar()
#     except Exception as e:
#         print("❌ Failed to fetch SCOPE_IDENTITY():", e)
#         return None
...
def get_login_data(username):
    try:
        with engine.connect() as conn:
            result = conn.execute(text("""
                    SELECT Email, First_Name, Physio_Password, Password_last_changed, Is_Admin, Preferred_Language
