from PySide6.QtWidgets import (QLabel, QLineEdit, QPushButton, QWidget, QMainWindow, QTableWidget, QToolButton, QTableWidgetItem,
                               QMessageBox, QTextEdit, QComboBox, QDateEdit, QCheckBox, QFormLayout, QApplication)
...
from TestParametersPage import Ui_TestParametersPage
from PreparationPage import Ui_PreparationPage
from ResultsAnalysisPage import Ui_ResultsAnalysisPage
from AdminPage import Ui_AdminPage
from RegisterNewPhysiotherapist import Ui_RegisterNewPhysiotherapist
from RemoveExistingPhysiotherapists import Ui_RemoveExistingPhysiotherapists
from PatientHistoryPage import Ui_PatientHistoryPage
from FirstUsePage import Ui_FirstUsePage
from CheckSystemPage import Ui_CheckSystemPage
from ResetPasswordPage import Ui_ResetPasswordPage
...
class Physiotherapist_Utils(QObject):
    def __init__(self, ui):
        ...
        self.eye_open = "images/eye_open.png"
        self.eye_closed = "images/eye_closed.png"
    ...
    def setup_login_ui(self):
        ...
        self.language_manager.save_widget_positions_to_json(Ui_LoginPage, "Positions/LoginPage.json")
        self.translate_login_ui()
        self.language_manager.shift_login(self.current_language, self.login_ui)
    ...
    def setup_reset_password_ui(self):
        self.current_language = DatabaseUtils.get_preferred_language(self.current_user.get("email"))
        self.reset_ui = Ui_ResetPasswordPage()
        self.reset_ui.setupUi(self.parent_window)
        self.language_manager.save_widget_positions_to_json(Ui_ResetPasswordPage, "Positions/ResetPasswordPage.json")
        self.translate_reset_password_ui()
        self.language_manager.shift_reset(self.current_language, self.reset_ui)
    ...
    def setup_first_use_ui(self):
        ...
        self.language_manager.save_widget_positions_to_json(Ui_FirstUsePage, "Positions/FirstUsePage.json")
        self.translate_first_use_ui()
        self.language_manager.shift_first_use(self.current_language, self.first_use_ui)
        ...
        self.first_use_ui.comboBox_Gender.clear()
        self.first_use_ui.comboBox_Gender.addItem(self.translator.tr("gender_male"), userData="Male")
        self.first_use_ui.comboBox_Gender.addItem(self.translator.tr("gender_female"), userData="Female")
        self.first_use_ui.comboBox_Gender.addItem(self.translator.tr("gender_other"), userData="Other")
    ...
    def setup_admin_ui(self):
        ...
        self.language_manager.save_widget_positions_to_json(Ui_AdminPage, "Positions/AdminPage.json")

        self.translate_admin_ui()
        self.language_manager.shift_admin(self.current_language, self.admin_ui)
    ...
    def setup_register_ui(self):
        ...
        self.language_manager.save_widget_positions_to_json(Ui_RegisterNewPhysiotherapist, "Positions/RegisterNewPhysiotherapist.json")
        self.translate_register_ui()
        self.language_manager.shift_register(self.current_language, self.register_ui)
        ...
        self.register_ui.ReturnButton.clicked.connect(self.load_admin_page)
        ...
        self.register_ui.comboBox_Gender.clear()
        self.register_ui.comboBox_Gender.addItem(self.translator.tr("gender_male"), userData="Male")
        self.register_ui.comboBox_Gender.addItem(self.translator.tr("gender_female"), userData="Female")
        self.register_ui.comboBox_Gender.addItem(self.translator.tr("gender_other"), userData="Other")
    ...
    def setup_remove_ui(self):
        ...
        self.language_manager.save_widget_positions_to_json(Ui_RemoveExistingPhysiotherapists, "Positions/RemoveExistingPhysiotherapists.json")
        self.translate_remove_ui()
        self.language_manager.shift_remove(self.current_language, self.remove_ui)
    ...
    def setup_test_parameters_ui(self):
        self.current_language = DatabaseUtils.get_preferred_language(self.current_user.get("email"))
        self.test_parameters_ui = Ui_TestParametersPage()
        self.test_parameters_ui.setupUi(self.parent_window)
        self.language_manager.save_widget_positions_to_json(Ui_TestParametersPage, "Positions/TestParametersPage.json")
        self.translate_test_parameters_ui()
        self.language_manager.shift_test_parameters(self.current_language, self.test_parameters_ui)
        ...
        self.test_parameters_ui.comboBox_Gender.clear()
        self.test_parameters_ui.comboBox_Gender.addItem(self.translator.tr("gender_male"), userData="Male")
        self.test_parameters_ui.comboBox_Gender.addItem(self.translator.tr("gender_female"), userData="Female")
        self.test_parameters_ui.comboBox_Gender.addItem(self.translator.tr("gender_other"), userData="Other")
        self.test_parameters_ui.comboBox_Gender.setCurrentIndex(-1)

        self.test_parameters_ui.comboBox_Has_Fallen.clear()
        self.test_parameters_ui.comboBox_Has_Fallen.addItem(self.translator.tr("fallen_yes"), userData="Yes")
        self.test_parameters_ui.comboBox_Has_Fallen.addItem(self.translator.tr("fallen_no"), userData="No")
        self.test_parameters_ui.comboBox_Has_Fallen.setCurrentIndex(-1)

        for error_label in [self.test_parameters_ui.error_Patient_Id, self.test_parameters_ui.error_Gender, self.test_parameters_ui.error_Shoe_Width,
        ...
        if self.current_patient_id:
            self.test_parameters_ui.lineEdit_Patient_ID.setText(self.current_patient_id)
    ...
    def setup_check_system_ui(self):
        ...
        self.language_manager.save_widget_positions_to_json(Ui_CheckSystemPage, "Positions/CheckSystemPage.json")
        self.translate_check_system_ui()
        self.language_manager.shift_check_system(self.current_language, self.check_system_ui)
    ...
    def setup_preparation_ui(self):
        ...
        self.language_manager.save_widget_positions_to_json(Ui_PreparationPage, "Positions/PreparationPage.json")
        self.translate_preparation_ui()
        self.language_manager.shift_preparation(self.current_language, self.preparation_ui)
    ...
    def setup_result_ui(self):
        ...
        self.language_manager.save_widget_positions_to_json(Ui_ResultsAnalysisPage, "Positions/ResultsAnalysisPage.json")
        self.translate_results_analysis_ui()
        self.language_manager.shift_result(self.current_language, self.result_ui)
        ...
        self.result_ui.tableWidget_Results.verticalHeader().setDefaultSectionSize(42)
    ...
    def setup_patient_history_ui(self):
        ...
        self.language_manager.save_widget_positions_to_json(Ui_PatientHistoryPage, "Positions/PatientHistoryPage.json")
        self.translate_patient_history_ui()
        self.language_manager.shift_patient_hisory(self.current_language, self.patient_hisory_ui)

        self.gm_gauge = GraphManager(self.patient_hisory_ui.widget_Gauge_Canvas)
        self.gm_fall = GraphManager(self.patient_hisory_ui.widget_FallRisk)
        self.gm_step = GraphManager(self.patient_hisory_ui.widget_StepCount)
        self.gm_error = GraphManager(self.patient_hisory_ui.widget_ErrorSteps)
    ...
    def load_result_page(self, pelvic_width, shoe_width, has_fall_last_year):
        ...
        # Step 1: Retrieving the latest comment and its date
        last_note_text, _ = DatabaseUtils.get_last_note(self.current_patient_id)

        # Step 2: Note now entered
        new_note = self.current_notes

        # Step 3: If the new comment is the same as the previous comment - we will enter None, otherwise we will enter it
        ...
        # Saving the test data in the database
    ...
    def validate_login(self, username, password):
        ...
        if result:
            email, first_name, db_password, password_last_changed, is_admin = result
    ...
    def handle_email_finished(self, ui):
        ...
        if row:
            ...
            index = ui.comboBox_Gender.findData(gender)
            ui.comboBox_Gender.setCurrentIndex(index if index != -1 else -1)
    ...
    def register_physiotherapist(self, email, username, first_name, last_name, gender, dob, password, is_admin):
        ...
        try:
            ...
            QMessageBox.warning(None, "Weak Password", str(e))
            return
    ...
    def handle_registration(self, ui):
        ...
        ui.error_Password_Confirmation.hide()
        if hasattr(ui, "password_info_button"):
            ui.password_info_button.hide()
        ...
        is_admin = ui.checkBox_Is_Admin.isChecked() if ui == self.register_ui else "1"
        ...
        if password and not self.is_strong_password(password):
            ui.error_Password.setText(self.translator.tr("error_Password_not_strong_enough"))
            ui.error_Password.show()
            valid = False
            # Explanation button
            self.setup_password_info_button(ui)
        if password != password_conf:
    ...
    def populate_physios(self):
        ...
        self.remove_ui.tableWidget_Physiotherapists.setRowCount(len(rows))
    ...
    def on_start_test_clicked(self):
        ...
        # === Gender ===
        if self.test_parameters_ui.comboBox_Gender.currentIndex() == -1:
            self.test_parameters_ui.error_Gender.setText(self.translator.tr("error_Gender"))
            self.test_parameters_ui.error_Gender.show()
            gender_approved = False
        ...
        if id_approved and gender_approved and shoe_approved and pelvic_approved and has_fallen_approved:
    ...
    def autofill_patient_data(self):
        ...
        if info:
            self.current_patient_id = patient_id
            self.test_parameters_ui.error_Patient_Id.hide()
            self.test_parameters_ui.error_Gender.hide()
            self.test_parameters_ui.error_Shoe_Width.hide()
            self.test_parameters_ui.error_Pelvic_Width.hide()
            self.test_parameters_ui.error_Patient_Has_Fallen.hide()

            # --- Gender ---
            gender_value = info["gender"].strip().capitalize()
            index = self.test_parameters_ui.comboBox_Gender.findData(gender_value)
            self.test_parameters_ui.comboBox_Gender.setCurrentIndex(index if index != -1 else -1)
    ...
    def export_history_to_csv(self, patient_id: int):
        ...
        with open(filename, mode='w', newline='', encoding='utf-8-sig') as file:
            ...
            # Writing all rows in a date range
    ...
    def setup_password_visibility(self, ui):
        ...
        ui.icon_eye_open = QIcon(self.eye_open)
        ui.icon_eye_closed = QIcon(self.eye_closed)
        ...
        if hasattr(ui, 'lineEdit_Password_Confirmation'):
            ...
            ui.lineEdit_Password_Confirmation.addAction(ui.eye_action_conf, QLineEdit.ActionPosition.TrailingPosition)
    ...
    def show_password_requirements(self):
        QMessageBox.information(
            None,
            self.translator.tr("error_Password_requirements_title"),
            self.translator.tr("error_Password_requirements_full")
        )
    ...
    def filter_results_by_date(self):
        ...
        df = pd.DataFrame(results, columns=[
            "Test Date", "Fall Risk", "Step Count", "Error Steps",
            "Start Time", "End Time", "Test Time"
        ])

        if not df.empty:
            self.patient_hisory_ui.widget_Gauge_Canvas.setFixedSize(350, 305)
            self.gm_gauge.plot_success_gauge_kpi_from_df(
                df,
            ...
            self.gm_fall.plot_variable_over_time_from_df(df, "Fall Risk", translator=self.translator)
            self.gm_step.plot_variable_over_time_from_df(df, "Step Count", translator=self.translator)
            self.gm_error.plot_variable_over_time_from_df(df, "Error Steps", translator=self.translator)

        else:
            # Reset KPI
            self.patient_hisory_ui.label_Gauge_Title.setText("")
            self.patient_hisory_ui.label_Gauge_Percentage.setText("")

            # Reset all graphs
            self.gm_gauge.clear_plot("gauge")
            self.gm_fall.clear_plot("Fall Risk")
            self.gm_step.clear_plot("Step Count")
            self.gm_error.clear_plot("Error Steps")
    ...
    def set_language_combobox(self, combobox):
        preferred_lang = DatabaseUtils.get_preferred_language(self.current_user.get("email"))
        if preferred_lang == "English" or preferred_lang == "en":
            combobox.setCurrentIndex(0)
        elif preferred_lang == "עברית" or preferred_lang == "he":
    ...
    def handle_language_change(self, combobox, page_name: str):
        ...
        # Update translator and current language
        self.current_language = new_lang
        self.translator = Translator(language_code=new_lang)
        # Retranslate the current page
        match page_name:
            case "test_parameters":
                self.translate_test_parameters_ui()
                self.language_manager.shift_test_parameters(self.current_language, self.test_parameters_ui)
                self.retranslate_errors(self.test_parameters_ui)
            case "results":
                self.translate_results_analysis_ui()
                self.language_manager.shift_result(self.current_language, self.result_ui)
            case "register":
                self.translate_register_ui()
                self.language_manager.shift_register(self.current_language, self.register_ui)
                self.retranslate_errors(self.register_ui)
            case "admin":
                self.translate_admin_ui()
                self.language_manager.shift_admin(self.current_language, self.admin_ui)
            case "first_use":
                self.translate_first_use_ui()
                self.language_manager.shift_first_use(self.current_language, self.first_use_ui)
                self.retranslate_errors(self.first_use_ui)
            case "check_system":
                self.translate_check_system_ui()
                self.language_manager.shift_check_system(self.current_language, self.check_system_ui)
            case "remove":
            ...
            case "reset":
                self.translate_reset_password_ui()
                self.language_manager.shift_reset(self.current_language, self.reset_ui)
                self.retranslate_errors(self.reset_ui)
            case _:  # Default
                print(f"⚠️ Page '{page_name}' not recognized.")

    def setup_password_info_button(self, ui):
        if ui.lineEdit_Password.text() and not self.is_strong_password(ui.lineEdit_Password.text()):
            if not hasattr(ui, "password_info_button"):
                ui.password_info_button = QToolButton(ui.centralwidget)
                ui.password_info_button.setIcon(QIcon("images/info_icon.png"))
                ui.password_info_button.setCursor(Qt.PointingHandCursor)
                ui.password_info_button.setToolTip(self.translator.tr("error_Password_requirements_title"))
                ui.password_info_button.setStyleSheet("background-color: transparent; border: none;")
                ui.password_info_button.setAutoRaise(True)
                ui.password_info_button.clicked.connect(self.show_password_requirements)
                ui.password_info_button.setFixedSize(20, 20)

            QTimer.singleShot(1, lambda: self.reposition_password_info_button(ui))
            ui.password_info_button.show()

    def reposition_password_info_button(self, ui):
        if hasattr(ui, "password_info_button") and hasattr(ui, "error_Password"):
            error_geom = ui.error_Password.geometry()
            ui.password_info_button.move(error_geom.x() + error_geom.width() + 5, error_geom.y())
    ...
    def translate_check_system_ui(self):
        t = self.translator
        self.check_system_ui.label_Check_System_Page.setText(t.tr("check_system_title"))
        self.check_system_ui.ExplorerButton.setText(t.tr("explorer_button"))
        self.check_system_ui.DiagnosticButton.setText(t.tr("diagnostic_button"))
        self.check_system_ui.LogOffButton.setText(t.tr("logoff_button"))
        self.check_system_ui.ReturnButton.setText(t.tr("return_button"))
    ...
    def translate_first_use_ui(self):
        ...
        self.setup_password_info_button(self.first_use_ui)

    def translate_login_ui(self):
        t = self.translator
        self.login_ui.label_LoginPage.setText(t.tr("login_title"))
        self.login_ui.label_goodbye_user.setText(t.tr("Goodbye_User_Dynamic", name=self.current_user["first_name"]))
    ...
    def translate_preparation_ui(self):
        ...
        label = self.preparation_ui.label_Instructions
        self.preparation_ui.label_Instructions.setWordWrap(True)
        self.preparation_ui.label_Instructions.setTextFormat(Qt.PlainText)
        self.preparation_ui.label_Instructions.setText('\u202B' + self.translator.tr("preparation_instructions"))
    ...
    def translate_register_ui(self):
        ...
        current_gender_index = self.register_ui.comboBox_Gender.currentIndex()
        ...
        self.register_ui.comboBox_Gender.setCurrentIndex(current_gender_index)

        self.register_ui.AddButton.setText(t.tr("add_button"))
        self.register_ui.LogOffButton.setText(t.tr("logoff_button"))
        self.register_ui.ReturnButton.setText(t.tr("return_button"))
        ...
        if not self.register_ui.lineEdit_Password.text():
            self.register_ui.error_Password.setText(t.tr("error_Password"))
        else:
            self.register_ui.error_Password.setText(t.tr("error_Password_not_strong_enough"))
        self.register_ui.error_Password_Confirmation.setText(t.tr("error_password_confirmation"))

        self.setup_password_info_button(self.register_ui)

    def translate_remove_ui(self):
        t = self.translator
        self.remove_ui.label_Remove_Existing_Physiotherapist.setText(t.tr("remove_title"))

        self.remove_ui.RemoveButton.setText(t.tr("remove_button"))
        self.remove_ui.ReturnButton.setText(t.tr("return_button"))
    ...
    def translate_results_analysis_ui(self):
        t = self.translator
        self.result_ui.label_Result_Page.setText(t.tr("results_title"))

        self.result_ui.NewTestButton.setText(t.tr("new_test_button"))
        self.result_ui.HistoryResultsButton.setText(t.tr("history_results_button"))
        self.result_ui.LogOffButton.setText(t.tr("logoff_button"))

        self.result_ui.tableWidget_Results.horizontalHeaderItem(0).setText(t.tr("results_column_value"))
        rows = [
        ...
        for i, key in enumerate(rows):
            self.result_ui.tableWidget_Results.verticalHeaderItem(i).setText(t.tr(key))
    ...
    def translate_test_parameters_ui(self):
        ...
        self.test_parameters_ui.label_welcome_user.setText(t.tr("Welcome_User_Dynamic", name=self.current_user["first_name"]))
        ...
        # === Gender ===
        current_gender = self.test_parameters_ui.comboBox_Gender.currentData()
        self.test_parameters_ui.comboBox_Gender.setItemText(0, self.translator.tr("gender_male"))
        self.test_parameters_ui.comboBox_Gender.setItemText(1, self.translator.tr("gender_female"))
        self.test_parameters_ui.comboBox_Gender.setItemText(2, self.translator.tr("gender_other"))
        index_Gender = self.test_parameters_ui.comboBox_Gender.findData(current_gender)
        if current_gender is None:
            self.test_parameters_ui.comboBox_Gender.setCurrentIndex(-1)
        else:
            self.test_parameters_ui.comboBox_Gender.setCurrentIndex(index_Gender)

        # === Has Fallen Last Year ===
        current_has_fallen = self.test_parameters_ui.comboBox_Has_Fallen.currentData()
        self.test_parameters_ui.comboBox_Has_Fallen.setItemText(0, self.translator.tr("fallen_yes"))
        self.test_parameters_ui.comboBox_Has_Fallen.setItemText(1, self.translator.tr("fallen_no"))
        index_Has_Fallen = self.test_parameters_ui.comboBox_Has_Fallen.findData(current_has_fallen)
        if current_has_fallen is None:
            self.test_parameters_ui.comboBox_Has_Fallen.setCurrentIndex(-1)
        else:
            self.test_parameters_ui.comboBox_Has_Fallen.setCurrentIndex(index_Has_Fallen)
        ...
        if not self.test_parameters_ui.lineEdit_Patient_ID.text():
            self.test_parameters_ui.error_Patient_Id.setText(t.tr("error_Patient_Id_length"))
        else:
            self.test_parameters_ui.error_Patient_Id.setText(t.tr("error_Patient_Id_isdigit"))
        ...
        self.test_parameters_ui.error_Patient_Has_Fallen.setText(t.tr("error_Patient_Has_Fallen"))

    def retranslate_errors(self, ui):
        t = self.translator

        # Register / First Use / Reset pages
        if hasattr(ui, "error_Email") and ui.error_Email.isVisible():
            email = ui.lineEdit_Email.text().strip()
            if "@" not in email:
                ui.error_Email.setText(t.tr("error_Email@"))
            elif DatabaseUtils.email_exists(email):
                ui.error_Email.setText(t.tr("error_Email_Already_Exist"))
            else:
                ui.error_Email.setText(t.tr("error_email"))  # Default

        if hasattr(ui, "error_Username") and ui.error_Username.isVisible():
            ui.error_Username.setText(t.tr("error_Username_Already_Exist"))

        if hasattr(ui, "error_First_Name") and ui.error_First_Name.isVisible():
            ui.error_First_Name.setText(t.tr("error_First_Name"))

        if hasattr(ui, "error_Last_Name") and ui.error_Last_Name.isVisible():
            ui.error_Last_Name.setText(t.tr("error_Last_Name"))

        if hasattr(ui, "error_Gender") and ui.error_Gender.isVisible():
            ui.error_Gender.setText(t.tr("error_Gender"))

        if hasattr(ui, "error_Password") and ui.error_Password.isVisible():
            password = ui.lineEdit_Password.text().strip()
            if not password:
                ui.error_Password.setText(t.tr("error_Password"))
            elif not self.is_strong_password(password):
                ui.error_Password.setText(t.tr("error_Password_not_strong_enough"))
            else:
                ui.error_Password.setText(t.tr("error_Password"))  # Default

        if hasattr(ui, "error_Password_Confirmation") and ui.error_Password_Confirmation.isVisible():
            ui.error_Password_Confirmation.setText(t.tr("error_Password_Confirmation"))

        # Test Parameters Page
        if hasattr(ui, "error_Patient_Id") and ui.error_Patient_Id.isVisible():
            patient_id = ui.lineEdit_Patient_ID.text().strip()
            if not patient_id:
                ui.error_Patient_Id.setText(t.tr("error_Patient_Id_length"))
            elif not patient_id.isdigit():
                ui.error_Patient_Id.setText(t.tr("error_Patient_Id_isdigit"))
            elif len(patient_id) != 9:
                ui.error_Patient_Id.setText(t.tr("error_Patient_Id_length"))

        if hasattr(ui, "error_Pelvic_Width") and ui.error_Pelvic_Width.isVisible():
            text = ui.lineEdit_Pelvic_Width.text().strip()
            range_text = DatabaseUtils.get_valid_range(4)
            min_val, max_val = DatabaseUtils.parse_range(range_text)
            if not text:
                ui.error_Pelvic_Width.setText(t.tr("error_Pelvic_Width_Empty"))
            elif not text.replace('.', '', 1).isdigit():
                ui.error_Pelvic_Width.setText(t.tr("error_Pelvic_Width_numeric"))
            else:
                ui.error_Pelvic_Width.setText(t.tr("error_Pelvic_Width", min=min_val, max=max_val))

        if hasattr(ui, "error_Shoe_Width") and ui.error_Shoe_Width.isVisible():
            text = ui.lineEdit_Shoe_Width.text().strip()
            range_text = DatabaseUtils.get_valid_range(5)
            min_val, max_val = DatabaseUtils.parse_range(range_text)
            if not text:
                ui.error_Shoe_Width.setText(t.tr("error_Shoe_Width_Empty"))
            elif not text.replace('.', '', 1).isdigit():
                ui.error_Shoe_Width.setText(t.tr("error_Shoe_Width_numeric"))
            else:
                ui.error_Shoe_Width.setText(t.tr("error_Shoe_Width", min=min_val, max=max_val))

        if hasattr(ui, "error_Patient_Has_Fallen") and ui.error_Patient_Has_Fallen.isVisible():
            ui.error_Patient_Has_Fallen.setText(t.tr("error_Patient_Has_Fallen"))
